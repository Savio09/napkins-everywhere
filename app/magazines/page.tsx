...
export default function Magazine() {
  ...
  const { data: posts } = useMagazines("populate=cover_img&fields=*");
  ...
  const pastIssues =
    posts && latestIssue
      ? posts.data.filter((magazine) => magazine.id !== latestIssue.id)
      : (posts?.data ?? []);

  const currentPost = pastIssues.slice(firstIdx, lastIdx);
              ...
              >
                {currentPost.map((magazine, index) => (
                  <div
                    ...
                    style={{ transitionDelay: isTransitioning ? "0ms" : `${index * 150}ms` }}
                  >
                    <CardIssues
