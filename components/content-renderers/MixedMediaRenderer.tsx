...
export default function MixedMediaRenderer({ entry, isVisible }: MixedMediaRendererProps) {
  return (
    <div className="w-full">
      ...
      {entry.media_files && entry.media_files.length > 1 && (
        <div
          ...
          className={`animate-on-scroll transform transition-all duration-1000 delay-500 ${
            isVisible["media-showcase"] ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
          }`}
        >
                  ...
                  <div className="relative">
                    <Image
                      ...
                      className={`w-full object-cover transform transition-transform duration-700 group-hover:scale-110 ${
                        isLarge ? "h-64 md:h-80" : isMedium ? "h-40" : "h-32"
                      }`}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="absolute bottom-3 left-3 text-white">
                        <p className="text-sm font-medium">{media.name || `Media ${index + 2}`}</p>
                        <p className="text-xs opacity-80">{media.ext?.toUpperCase() || "IMAGE"}</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div
        ...
        className={`animate-on-scroll mt-16 transform transition-all duration-1000 delay-700 ${
          isVisible["interactive-features"] ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}
      >
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-8 border border-blue-100">
          <h4 className="text-xl font-bold text-[#0070ae] mb-4 text-center">Explore This Content</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            {[
              { icon: "📖", title: "Full Article", desc: "Read the complete story" },
              { icon: "🖼️", title: "Gallery View", desc: "Browse all media" },
              { icon: "🔗", title: "Share", desc: "Share this content" },
            ].map(({ icon, title, desc }) => (
              <div
                key={title}
