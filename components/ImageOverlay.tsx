...
interface ImageOverlayProps {
  isLatestIssueBgLoaded: boolean;
  coverImageUrl: string | null | undefined;
  latestIssueLink: string;
  latestIssueTitle: string | null | undefined;
  sectionTitle: string;
  linkText?: string;
  children?: ReactNode;
}
  ...
  return (
    <section
      className={`
        ...
        ${isLatestIssueBgLoaded ? "opacity-100 before:opacity-90" : "opacity-0 before:opacity-0"}
      `}
      style={{
        backgroundImage:
          isLatestIssueBgLoaded && coverImageUrl ? `url('${coverImageUrl}')` : "none",
      }}
    >
      <div
        className={`
        ...
        `}
      >
        <h3 className="text-xl md:text-2xl font-semibold mb-2">{sectionTitle}</h3>
        <h1 className="text-4xl md:text-5xl font-bold mb-6">{latestIssueTitle || "Loading..."}</h1>
        <div className="cta">
          <Link
