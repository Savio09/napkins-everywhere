...
import { useEntry, useRelatedEntries } from "@/hooks/useStrapi";
...
export default function EntryDetail() {
  ...
  const { data: entryData, error: entryError } = useEntry(entrySlug);
  const entry = entryData?.data?.[0] ?? null;
  const magazine = entry?.magazine ?? null;

  const { data: relatedEntriesData } = useRelatedEntries(magazine?.id, entry?.id);
    ...
    setTimeout(() => {
      ...
      if (backNavEl) {
        ...
        if (rect.top < window.innerHeight + 200) {
          setIsVisible((prev) => ({ ...prev, "back-nav": true }));
        }
      }
    }, 500);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-bold text-red-500 mb-4">Entry Not Found</h1>
          <Link href={`/magazines/${magazineSlug}`} className="text-[#0070ae] hover:underline">
            Back to Magazine
          </Link>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 pt-8">
        <nav className="flex items-center space-x-2 text-sm text-gray-600 mb-8">
          <Link href="/magazines" className="hover:text-[#0070ae] transition-colors">
            Magazines
          </Link>
          <span>/</span>
          <Link
      ...
      <section className="max-w-7xl mx-auto px-4 mb-8">
        <div className="mb-8">
          ...
          <div className="text-lg text-gray-600 animate-fade-in-delay">
            {entry.author && <span className="font-medium">by {entry.author}</span>}
      ...
      <article className="max-w-7xl mx-auto px-4 pb-16">
        ...
        {relatedEntriesData && relatedEntriesData.data && relatedEntriesData.data.length > 0 && (
          <div
            id="related-entries"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {relatedEntriesData.data.slice(0, 4).map((relatedEntry) => (
                <Link
                  key={relatedEntry.id}
                  href={`/magazines/${relatedEntry.magazine?.slug || magazineSlug}/${relatedEntry.slug}`}
                ...
                >
                  <div className="bg-white rounded-lg shadow-md p-6 transform transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border-l-4 border-[#0070ae] hover:border-l-8">
                    <div className="flex items-start justify-between">
                      ...
                      {relatedEntry.media_files && relatedEntry.media_files.length > 0 && (
                        <div className="ml-6 flex-shrink-0">
                          <Image
                            src={createLocalImageURL(relatedEntry.media_files[0].url)}
                            width={80}
                            height={80}
        ...
        <div
          ...
          className={`animate-on-scroll mt-16 text-center transform transition-all duration-700 delay-400 ${
            isVisible["back-nav"] ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
        >
          <Link
            href={`/magazines/${magazineSlug}`}
            className="inline-flex items-center px-6 py-3 bg-[#0070ae] text-white rounded-lg font-medium hover:bg-blue-700 transition-all duration-300 transform hover:scale-105 hover:shadow-lg"
          >
